...
直接返回预设文件名（如 "hello"），不要加任何解释。"""
    ...
    def get_response_stream(self, user_input: str, image_path: str = None):
        ...
        try:
            ...
            # 调用流式API
            messages = [
                {"role": "system", "content": self.TEXT_GENERATION_PROMPT}
    ...
    def run(self):
        try:
            ...
            if use_audio_mode:
                ...
                try:
                    emotion_match = re.match(r'\[(\w+)\]', full_content)
                    if emotion_match:
                        emotion = emotion_match.group(1)
                    ...
                    # 如果解析失败，使用默认值
                    emotion = "normal"
                    chinese_text = "（系统错误，请稍后再试）"
                    japanese_text = "（システムエラー）"
                ...
                # 在非音频模式下，生成日语文本并发射信号
                if full_text and not self.enable_audio:
                    try:
                        # 简单的语言检测：如果包含日语字符，当作日语
                        has_japanese = bool(re.search(r'[\u3040-\u30ff\u3400-\u4dbf\u4e00-\u9fff\uff66-\uff9f]', full_text))
                        if has_japanese:
                            japanese_text = full_text
                        else:
                            # 否则翻译为日语
                            japanese_text = self.ai_manager._translate_to_japanese(full_text)
                        
                        print(f"[非音频模式] 生成日语文本: {japanese_text}")
                        print(f"[非音频模式] 准备发射 japanese_text_ready 信号")
                        self.japanese_text_ready.emit(japanese_text)
                        print(f"[非音频模式] japanese_text_ready 信号已发射")
                    except Exception as e:
                        print(f"[非音频模式] 生成日语文本失败: {e}")
                        import traceback
                        traceback.print_exc()
                else:
                    print(f"[非音频模式] 跳过日语文本生成，因为 full_text={bool(full_text)}, enable_audio={self.enable_audio}")

                self.response_complete.emit(full_text, emotion, "")
...
class SettingsDialog(QDialog):
    ...
    def __init__(self, parent=None, current_api_key="", vocu_api_key="", vocu_voice_id="", audio_mode=True, current_max_tokens=200):
        super().__init__(parent)
        ...
        # 说明文字
        info_label = QLabel("模型: doubao-seed-1-6-lite-251015")
...
class MainWindow(QMainWindow):
    ...
    def open_settings(self):
        ...
        if result == QDialog.DialogCode.Accepted:
            ...
            print(f"保存Vocu设置...")
            self._save_vocu_settings(settings["vocu_api_key"], settings["vocu_voice_id"], settings["audio_mode"], settings["max_tokens"])
            # 重新初始化音频生成器
    ...
    def _save_vocu_settings(self, api_key: str, voice_id: str, audio_mode: bool = True, max_tokens: int = 200):
        """保存Vocu设置到QSettings"""
        from PyQt6.QtCore import QSettings
        ...
        try:
            ...
            print(f"Vocu配置已保存: api_key={'*' * len(api_key) if api_key else '空'}, voice_id={voice_id or '空'}, audio_mode={audio_mode}, max_tokens={max_tokens}")
        except Exception as e:
